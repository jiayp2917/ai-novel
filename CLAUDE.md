...
普通用户可见的前端界面文案默认使用中文，并提供明确的成功、失败和进度反馈。非特殊情况不要新增英文按钮、标题、提示、空状态或错误说明。

允许保留英文的范围仅限产品名、包名、供应商名、模型名、技术名词、代码标识、环境变量、命令、路径、API 字段、JSON/调试字段、用户输入、导入作品原文、原始模型/错误输出，以及第三方库不可控的浏览器或系统提示。
