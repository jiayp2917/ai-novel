...
仓库内 `skills/` 只保留通用生产规则。特定作品的风格、审核清单和长期偏好应放在 `D:\chat\novel-workspaces\作品名\skills\`，例如本机《机制怪？我是数值怪》的作品级 skill 已迁到对应作品工作区，不随系统代码提交。
...
最近一次复制工作区第 1-10 章 Agnes dry-run 报告：
...
```powershell
python -m backend.tools.package_system --out D:\chat\packages\novel_editor_system.zip
```
