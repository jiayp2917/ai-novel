...
补充审计、架构和 UI 重构记录目前仍保存在 `docs/` 中，作为开发过程材料。后续开源前建议由维护者确认是否并入三份核心文档，或作为内部过程文档移出公开目录。

## 当前核验状态

最近一次本地核验显示全绿：后端 `pytest` 206、前端 `vitest` 235、前端构建、Playwright E2E 27/27 全部通过。此前 v0.8 重构引入的 7 个 e2e 回归已修复（commit a0f3792，根因与修复见 [ai-novel 开发计划](docs/ai-novel开发计划.md)）。
