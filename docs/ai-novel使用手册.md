...
- 前端内置三套主题：清风微动、星空鎏金、白丝质感。主题只改变视觉氛围，不改变写作、审核、diff、发布门和模型配置流程。
...
- 读取作品状态、章节、素材和日志可以用于普通查看。
- 打开工作区、扫描、创建批注、保存正文版本、生成 proposal、触发 AI、推进流水线、人工检查和正式写回都属于状态变更，默认只允许本机访问。
- 配置 `ADMIN_API_TOKEN` 后，前端必须通过 `VITE_ADMIN_API_TOKEN` 携带同一个 token，否则状态变更会被拒绝。

这些变量都属于本机配置，不应写入公开文档、报告、日志或提交记录。界面只应显示“已配置/未配置”等状态，不回显密钥明文。

模型配置界面采用“模型档案 + 角色分配”：

- 模型档案保存供应商、模型名、接口地址、密钥环境变量、输出上限和 JSON 支持能力。
- 写作、检查、修订、小修、章纲、结构修订、记忆、长上下文和高风险判断等角色可分别分配模型。
- 内置档案只读；自定义档案在被角色使用时不能删除。
- 应用内密钥只显示状态，不显示明文；真实密钥仍建议保存在本机环境或本机加密存储中。
...
旧 `00/01/02/03` 目录作品需要先用迁移工具转换为 `content/*`，再在界面中打开并扫描。主应用不再自动扫描旧目录，避免开源版本长期维护多套路径规则。
...
主题切换入口位于顶部工具栏。若旧浏览器缓存中保存过早期主题值，应用会自动迁移到当前三主题之一；主题图片只作为背景材质，不影响正文阅读和操作按钮。
