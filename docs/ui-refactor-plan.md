...
> 本节是 **2026-06-17 v0.6 时的现状盘点快照**（"✗ 未启动"等状态为盘点当时所见，行号亦为当时实测）。
> **最新执行结果一律以 §0.8 为准**；本节保留作历史依据，其中的旧行号（`ReaderPanel.tsx` 425 行、`PipelineView.tsx` 735 行、`ModelsView.tsx` 782 行、`WorkflowActions.tsx` 597 行等）已随 v0.8 拆分全部失效。
